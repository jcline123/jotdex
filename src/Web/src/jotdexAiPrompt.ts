...
In the editor, each code box edits inline like a normal code box (language, Copy, Edit, tab indent). Uncommon actions (Insert snippet, Save as snippet) live under the ☰ overflow menu on the code-box bar. **Edit** opens the advanced CodeMirror dialog (line numbers, fold gutter, find, Word wrap, Show whitespace, Insert snippet / Save as snippet on the dialog bar, Ctrl+Space snippet completions, parse-only diagnostics). PowerShell and JSON can show syntax / style hints — that does **not** run your code. When PSScriptAnalyzer is available on the server, PowerShell may also show best-practice warnings.

**Reusable snippets** live as vault files under a reserved \`Snippets/\` folder with front matter \`jotdex_type: code-snippet\`, plus \`jotdex_trigger\` (shortcut keyword for Ctrl+Space) and \`jotdex_language\`. The fenced code block in the note body is the snippet body. Click a snippet to edit it in the main note pane (full editor). Stored under \`Snippets/\`, separate from your notes list. **Insert snippet** / **Save as snippet** are under the ☰ menu on code boxes and in the **Edit** dialog.
