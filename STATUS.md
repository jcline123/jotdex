...
- Portable **1.3.4** (`v1.3.4`) — color/size spans reopen as color (not raw HTML); Todos rail strips tags from note checkbox titles. Live vault Markdown was **not** rewritten. Rollback is the previous exe (1.3.3); keep `artifacts/jotdex-win-x64-1.3.3.zip` and `C:\JotdexBackupHold\jotdex-win-x64-1.3.3.zip`.
